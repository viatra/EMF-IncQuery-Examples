...
public class AlphabeticallyAfterFriendMatcher extends BaseMatcher<AlphabeticallyAfterFriendMatch> {
  ...
  public AlphabeticallyAfterFriendMatch newMatch(final Student pS1, final Student pS2) {
    return AlphabeticallyAfterFriendMatch.newMatch(pS1, pS2);
  }
  ...
  protected AlphabeticallyAfterFriendMatch tupleToMatch(final Tuple t) {
    try {
    	return AlphabeticallyAfterFriendMatch.newMatch((school.Student) t.get(POSITION_S1), (school.Student) t.get(POSITION_S2));
  ...
  protected AlphabeticallyAfterFriendMatch arrayToMatch(final Object[] match) {
    try {
    	return AlphabeticallyAfterFriendMatch.newMatch((school.Student) match[POSITION_S1], (school.Student) match[POSITION_S2]);
    } catch(ClassCastException e) {
  ...
  protected AlphabeticallyAfterFriendMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return AlphabeticallyAfterFriendMatch.newMutableMatch((school.Student) match[POSITION_S1], (school.Student) match[POSITION_S2]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
