 ...
 * 
 * <p>Matches of the pattern will be represented as {@link SuperTypeOfMatch}.
 * 
 * <p>Original source:
 * <code><pre>
...
public class SuperTypeOfMatcher extends BaseMatcher<SuperTypeOfMatch> {
   ...
   * 
   */
  public static SuperTypeOfMatcher on(final ViatraQueryEngine engine) throws IncQueryException {
    // check if matcher already exists
    SuperTypeOfMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = new SuperTypeOfMatcher(engine);
  ...
  private final static Logger LOGGER = IncQueryLoggingUtil.getLogger(SuperTypeOfMatcher.class);
   ...
   */
  @Deprecated
  public SuperTypeOfMatcher(final ViatraQueryEngine engine) throws IncQueryException {
  ...
  protected SuperTypeOfMatch tupleToMatch(final Tuple t) {
    try {
    	return SuperTypeOfMatch.newMatch((EClass) t.get(POSITION_SUPER), (EClass) t.get(POSITION_SUB));
  ...
  protected SuperTypeOfMatch arrayToMatch(final Object[] match) {
    try {
    	return SuperTypeOfMatch.newMatch((EClass) match[POSITION_SUPER], (EClass) match[POSITION_SUB]);
    } catch(ClassCastException e) {
  ...
  protected SuperTypeOfMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return SuperTypeOfMatch.newMutableMatch((EClass) match[POSITION_SUPER], (EClass) match[POSITION_SUB]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
