...
public class IsInECoreMatcher extends BaseMatcher<IsInECoreMatch> {
   ...
   * 
   */
  public static IsInECoreMatcher on(final ViatraQueryEngine engine) throws IncQueryException {
   ...
   */
  @Deprecated
  public IsInECoreMatcher(final ViatraQueryEngine engine) throws IncQueryException {
  ...
  protected IsInECoreMatch tupleToMatch(final Tuple t) {
    try {
    	return IsInECoreMatch.newMatch((EClassifier) t.get(POSITION_ELEMENT));
  ...
  protected IsInECoreMatch arrayToMatch(final Object[] match) {
    try {
    	return IsInECoreMatch.newMatch((EClassifier) match[POSITION_ELEMENT]);
    } catch(ClassCastException e) {
  ...
  protected IsInECoreMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return IsInECoreMatch.newMutableMatch((EClassifier) match[POSITION_ELEMENT]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
